...
test("Deberian existir 221 tweets poco agresivos, con nivel 1", async () => {
  const numberOfTweets = await db.getNumberOfTweetsByLevel(1);
  expect(numberOfTweets).toBe(221);
});

test("Deberian existir 187 tweets muy agresivos, con nivel 5", async () => {
  const numberOfTweets = await db.getNumberOfTweetsByLevel(5);
  expect(numberOfTweets).toBe(187);
});

test("Deberian existir 592 tweets normales, con nivel entre 2 y 4", async () => {
